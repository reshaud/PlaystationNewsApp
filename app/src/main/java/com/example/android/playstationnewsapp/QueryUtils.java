...
public final class QueryUtils {
    ...
    //Query GuardianAPI for News data
    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            //Extract JSONObject associated with the key response
            ...
            for(int i = 0; i < resultsJSON.length(); i++){
                ...
                JSONObject currentJSONObject = resultsJSON.getJSONObject(i);

                //Get webTitle from JSON
                String webTitle = currentJSONObject.getString("webTitle");

                //Get sectionName from JSON
                String sectionName = currentJSONObject.getString("sectionName");

                //Get webPublicationDate from JSON
                String publishedDate = currentJSONObject.getString("webPublicationDate");

                //Get webURL from JSON
                String webURL = currentJSONObject.getString("webUrl");

                newsList.add(new News(webTitle,sectionName,publishedDate,webURL));
            }
